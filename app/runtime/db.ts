...
const dbConnection = mongoose.connect(connectionStr, { user: config.get('db.options.user'), pass: config.get('db.options.pass')})

mongoose.connection.on('error', err => logger.error('mongodb connect error:' + err))
...
for (let model of models) {
  if (model.startsWith('.') || model.endsWith('.ts')) continue
  const modelName = path.basename(model).split('.')[0]
  const modelSchema = require(path.join(modelsPath, model))
  db[modelName] = dbConnection.model(modelName, modelSchema.schema)
}
